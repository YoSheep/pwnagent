...
- 如果页面已经暴露站内真实跳转（例如带查询参数的内容页或详情页链接），优先继续跟进这些页面，不要反复做广域枚举或无根据的 SSRF/JWT 测试"""
...
class Brain:
    ...
    def _render_parallel_summary(tool_calls: list[ToolCall]):
        if len(tool_calls) > 1:
            names = ", ".join(Brain._tool_display_name(tc) for tc in tool_calls)
            console.print(f"[cyan]  → 并行调用 {len(tool_calls)} 个工具: {names}[/cyan]")

    @staticmethod
    def _tool_display_name(tool_call: ToolCall) -> str:
        action_input = tool_call.input if isinstance(tool_call.input, dict) else {}
        ...
        if tool_call.name in {"http_request", "page_intel"}:
            method = str(action_input.get("method", "GET")).upper()
            return f"{tool_call.name}({method} {path_preview})"
        if tool_call.name == "httpx_probe":
