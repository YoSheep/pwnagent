...
PwnAgent 是一个基于 Anthropic Claude 构建的 AI 驱动渗透测试框架。采用 **ReAct（Reason + Act）** 智能体架构，能够自主执行从信息收集到漏洞利用的完整渗透测试流程。内置 SafetyGuard 安全门卫机制，确保所有操作严格限定在授权范围内。

## 核心特性

- **单 Agent + 并行工具执行** — 基于 ReAct 循环，单个 Claude Agent 通过 `ThreadPoolExecutor` 并行调度多个工具，兼顾推理一致性与执行效率
...
│   ├── brain.py            # Claude API 交互（流式 + 多工具调用）
...
| 组件 | 技术 |
|------|------|
| LLM | Anthropic Claude（原生 tool_use 协议） |
| Agent 框架 | 原生 ReAct 实现（无 LangChain 依赖） |
| 流式通信 | Anthropic Streaming API（事件驱动） |
