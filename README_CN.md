...
| 类别 | 工具 |
|------|------|
| 信息收集 / 基线探测 | `nmap_scan`, `python_port_scan`, `httpx_probe`, `dirbust`, `dirsearch_init` |
| SQLMap 运行时 / 直接操作 | `sqlmap_init`, `sqlmap_scan_url`, `sqlmap_enumerate_databases`, `sqlmap_enumerate_tables`, `sqlmap_enumerate_columns`, `sqlmap_dump_table`, `sqlmap_get_banner`, `sqlmap_get_current_user`, `sqlmap_get_current_db`, `sqlmap_read_file`, `sqlmap_execute_command` |
| 报告 / 会话产物 | `generate_report`, `sessions`, `report` |

### 已移入后续计划（持续加固，不再标“已完善”）

- `page_intel`
- `http_request`, `login_form`, `upload_file`
- `sqli_scan`（Agent 封装层行为与收敛性仍在优化）
- `ssrf_scan`（异步新版需要更大样本实战验证）
- `xss_scan`（上下文判定与动态验证仍在调优）
...
PentestPilot 提供 MCP Server，可将所有安全测试工具直接暴露给 Claude Code。
...
| `dirbust` | 目录爆破 |
| `dirsearch_init` | 初始化/更新 dirsearch 运行时 |
...
| dirsearch | 路径发现 / 递归爆破 / 状态过滤 | 自动回退到内置 Python `dirbust` 引擎 |
...
在任意目录下创建 Python 文件，PentestPilot 会自动发现并注册：
...
PentestPilot 现在不再内置 scope 或 rate-limit 强制检查。如果你需要授权范围限制、安全目标白名单或执行节流，需要在你自己的工作流、包装层、代理层或部署环境中自行实现。
